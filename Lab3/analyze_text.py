...
def analyze(my_text):
    ...
    output_json = my_parser.api_call(my_text, properties={'annotators': 'tokenize,ssplit'})

    for idx, token in enumerate(output_json["sentences"][0]["tokens"]):
        start_offset = token['characterOffsetBegin']  # Begin offset of token.
        # End offset of token, without taking into account the last space asociated to the token
        end_offset = token['characterOffsetEnd'] - 1
        my_tree.nodes[idx+1]["start"] = start_offset
        my_tree.nodes[idx+1]["end"] = end_offset
